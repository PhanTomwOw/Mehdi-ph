    ...
    <div className="bg-card rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 group border border-border">
      <div className="relative overflow-hidden">
        ...
        <button 
            ...
            className={`absolute top-3 left-3 bg-black/40 backdrop-blur-sm p-2 rounded-full hover:bg-black/60 transition-all duration-200 z-10 ${favorited ? 'text-primary' : 'text-white'}`}
            aria-label={favorited ? 'Remove from favorites' : 'Add to favorites'}
        >
            <HeartIcon className="w-5 h-5" filled={favorited} />
        </button>

        <div className="absolute top-0 right-0 bg-primary text-primary-foreground px-3 py-1 m-3 rounded-full text-sm font-semibold flex items-center gap-1">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center text-muted-foreground mb-4">
          <LocationIcon className="w-5 h-5 mr-2 text-muted-foreground" />
